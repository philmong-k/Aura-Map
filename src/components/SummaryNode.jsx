...
const SummaryNode = ({ id, data, selected }) => {
  ...
  const summaryData = useMemo(() => {
    ...
    // 독립 노드: type이 tactical이고 parentId가 없는 노드 (mode가 note인 경우는 제외)
    const independentNodes = nodes.filter(n => n.type === 'tactical' && !n.parentId && n.data?.mode !== 'note');
    ...
    const groupList = groups.map(g => {
      // 해당 그룹을 부모로 가진 자식 노드 중 note 모드가 아닌 노드들만 필터링
      const children = nodes.filter(n => n.parentId === g.id && n.data?.mode !== 'note');
